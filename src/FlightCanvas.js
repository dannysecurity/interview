...
function FlightCanvas({ trajectory = [], width = 500, height = 300 }) {
  ...
  // convert simulation coordinates (meters) to canvas coordinates
  const convert = (pos) => {
    // assume x is horizontal meters, y is vertical meters
    // we map x to canvas x linearly from 0..maxX -> 0..width
    // and y to canvas y inverted (0 is ground) to height
